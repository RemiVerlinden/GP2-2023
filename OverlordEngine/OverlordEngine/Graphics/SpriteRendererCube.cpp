#include "stdafx.h"
#include "SpriteRendererCube.h"

void SpriteRendererCube::Initialize()
{
	//Effect
	m_pEffect = ContentManager::Load<ID3DX11Effect>(L"Effects/SpriteRendererCube.fx");
...
bool SpriteRendererCube::SpriteSortByTexture(const VertexSprite& v0, const VertexSprite& v1)
{
	return v0.TextureId < v1.TextureId;
}

bool SpriteRendererCube::SpriteSortByDepth(const VertexSprite& v0, const VertexSprite& v1)
{
	return v0.TransformData.z < v1.TransformData.z;
}

SpriteRendererCube::~SpriteRendererCube()
{
...
void SpriteRendererCube::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		HANDLE_ERROR(hr, L"SpriteRendererCube: CreateBuffer failed");
	}
...
void SpriteRendererCube::Draw(const SceneContext& sceneContext)
{
	if (m_Sprites.empty())
		return;
...
void SpriteRendererCube::AppendSprite(TextureData* pTexture, const XMFLOAT2& position, const XMFLOAT4& color, const XMFLOAT2& pivot, const XMFLOAT2& scale, float rotation, float depth)
{
	VertexSprite vertex{};
...
void SpriteRendererCube::DrawImmediate(const D3D11Context& d3dContext, ID3D11ShaderResourceView* pSrv, const XMFLOAT2& position, const XMFLOAT4& color, const XMFLOAT2& pivot, const XMFLOAT2& scale, float rotation)
{
	//Create Immediate VB
